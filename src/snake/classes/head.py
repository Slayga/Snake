...
class Head:
    def __init__(self,
        ...
        self.lbl_head.grid(column=self.start_pos,
                           row=self.start_pos,
                           sticky=N + S + W + E)

        self.tail = Tail(self.window, self, self.board, self.update_speed,
                         self.tail_color)
    ...
    def check_collision(self, tail: Tail, spike: object = None):
        ...
        if (self.x >= self.board.x_max or self.x < self.board.x_min
                or self.y >= self.board.y_max or self.y < self.board.y_min):
            self.alive = False
        # Check if head is turning in to itself when it has a tail
        elif (self.old_direction == self.opposite_dir(self.direction)
              and self.board.score > 1):
            self.alive = False

        # Check if head collide with the tail
        elif (lbls[self.y][self.x].cget("bg") == tail.color
              and self.board.score > 1):
            self.alive = False
